...
public class TmxMapLoader extends AsynchronousAssetLoader<TiledMap, TmxMapLoader.Parameters> {
    ...
    /** Loads the {@link com.badlogic.gdx.maps.tiled.TiledMap} from the given file. The file is resolved via the {@link com.badlogic.gdx.assets.loaders.FileHandleResolver} set in the
    ...
    public TiledMap load (String fileName, int x) {
        return load(fileName, new Parameters(), x);
    }
    ...
    public TiledMap load (String fileName, Parameters parameters, int x) {
        try {
            this.yUp = parameters.yUp;
            FileHandle tmxFile = resolve(fileName);
            root = xml.parse(tmxFile);
            ...
            DirectImageResolver imageResolver = new DirectImageResolver(textures);
            TiledMap map = loadTilemap(root, tmxFile, imageResolver, x);
            map.setOwnedResources(textures.values().toArray());
            return map;
        } catch (IOException e) {
            throw new GdxRuntimeException("Couldn't load tilemap '" + fileName + "'", e);
    ...
    /** Loads the map data, given the XML root element and an {@link com.badlogic.gdx.maps.ImageResolver} used to return the tileset Textures
     * @param root the XML root element
     * @param tmxFile the Filehandle of the tmx file
     * @param imageResolver the {@link com.badlogic.gdx.maps.ImageResolver}
     * @return the {@link com.badlogic.gdx.maps.tiled.TiledMap} */
    ...
    protected TiledMap loadTilemap (Element root, FileHandle tmxFile, ImageResolver imageResolver, int x) {
        TiledMap map = new TiledMap();
     ...
     * an {@link com.badlogic.gdx.maps.ImageResolver} used to retrieve the tileset Textures.
     *
     * <p>
    ...
    protected void loadTileLayer (TiledMap map, Element element, int xDisplacement) {
        if (element.getName().equals("layer")) {
            String name = element.getAttribute("name", null);
            int width = element.getIntAttribute("width", 0);
            int height = element.getIntAttribute("height", 0);
            ...
            boolean visible = element.getIntAttribute("visible", 1) == 1;
            float opacity = element.getFloatAttribute("opacity", 1.0f);
            TiledMapTileLayer layer = new TiledMapTileLayer(width+xDisplacement, height, tileWidth, tileHeight);
            layer.setVisible(visible);
            layer.setOpacity(opacity);
            layer.setName(name);
                            ...
                            for (int x = 0; x < width; x++) {
                                try {
                                    ...
                                    if (tile != null) {
                                        Cell cell = createTileLayerCell(flipHorizontally, flipVertically, flipDiagonally);
                                        cell.setTile(tile);
                                        layer.setCell(x+xDisplacement, yUp ? height - 1 - y : y, cell);
                                        ///////////////////////////////////////////////
                                    }
